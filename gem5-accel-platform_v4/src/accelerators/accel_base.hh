...
class AccelBase : public DmaDevice
{
  ...
  protected:
     ...
     *
     * Only used by the default beginCompute() implementation below. If a
     * subclass overrides beginCompute() directly instead (e.g. an RTL
     * cosimulation wrapper that steps a Verilated model one cycle at a
     * time), it does not need a meaningful override of this - the base
     * class provides a panic-on-call default.
     */
    virtual Cycles computeLatency(uint64_t lenBytes, uint64_t param0,
                                   uint64_t param1) const;

    /**
     * Hook called once FETCH (dmaRead) completes, responsible for
     * eventually calling finishCompute() to proceed to WRITEBACK.
     *
     * Default implementation: call computeLatency() once and schedule a
     * single event after that many cycles - this is what NpuAccel/
     * LpuAccel/IspAccel use today via their computeLatency() overrides,
     * and continues to work unchanged.
     *
     * Override this instead (typically *without* overriding
     * computeLatency()) to implement a fundamentally different compute
     * strategy - e.g. RTL cosimulation, which steps a Verilated model one
     * clock cycle at a time via its own recurring event and calls
     * finishCompute() only once the RTL asserts a "done" signal, rather
     * than computing a cycle count up front analytically.
     */
    virtual void beginCompute();

    /**
     * Subclasses with a custom beginCompute() call this once their
     * compute phase is done, to proceed to WRITEBACK exactly as the
     * default analytical path does.
     */
    void finishCompute() { onComputeDone(); }

    /** Read/write access to the DMA scratch buffer for subclasses that
     *  drive their own compute phase (e.g. RTL wrappers feeding/reading
     *  the Verilated model's ports directly from the fetched bytes). */
    uint8_t *scratchPtr() const { return scratch.get(); }
    uint64_t scratchLen() const { return lenReg; }
