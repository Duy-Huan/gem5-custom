...
class Expander:
    ...
    def conv(self, name, out_c, k, s, depthwise=False, groups=1,
             out_hw=None):
        """out_hw: optional (oh, ow) override, used by onnx_to_json.py to
        pass the *exact* output size it already computed from the real
        ONNX node's pads/strides/dilations, instead of trusting this
        function's own "pad = k//2" assumption (which is a reasonable
        default for hand-authored JSON configs like mobilenetv2.json, but
        not guaranteed to match an arbitrary real model's actual padding
        choice)."""
        oh, ow = out_hw if out_hw is not None else conv_out_hw(self.h, self.w, k, s)

        if (depthwise or groups == self.c) and out_c == self.c:
            # True depthwise, channel multiplier 1 (groups == Cin == Cout)
            # - by far the common case (MobileNet-style). Matches the
            # existing depthwise PrimOp cost model in prim_op_to_calls(),
            # which assumes N=1 per channel.
            self.ops.append(
            ...
            # General grouped conv (e.g. ResNeXt-style, 1 < groups < Cin):
            # `groups` independent dense convs, each consuming Cin/groups
            # input channels and producing Cout/groups output channels.
            # Not a single GEMM (like depthwise, groups partition the
            # reduction), but unlike depthwise each group's GEMM is still
            # a genuine multi-channel-wide dense op, not a degenerate N=1
            # one - so each group gets its own ordinary "dense" PrimOp.
            assert self.c % groups == 0 and out_c % groups == 0, (
                f"{name}: in_c={self.c} and out_c={out_c} must both be "
                f"divisible by groups={groups}"
            )
            in_per_group = self.c // groups
            out_per_group = out_c // groups
            for g in range(groups):
                self.ops.append(
                    PrimOp(f"{name}.g{g}", "dense", oh, ow, in_per_group,
                           out_per_group, k, k)
                )
    ...
    def reshape(self, h, w, c):
        """No-MAC state sync: used when a real model has a shape-changing
        op with no multiply-accumulates of its own (pooling, global
        average pool, flatten/reshape before a linear layer...) between
        two compute layers. Emits no PrimOp - purely updates the
        Expander's notion of the "current tensor shape" so the *next*
        conv()/linear() call computes the correct M/K/N. Required for
        onnx_to_json.py's output to be interpreted correctly - a real
        model's pooling/flatten ops are skipped (no MACs) but still
        change shape, and the flat JSON layer list has no other way to
        communicate that to this Expander (which does not itself replay
        the skipped ops)."""
        self.h, self.w, self.c = h, w, c
...
def expand_model(model):
    ...
    for layer in model["layers"]:
        ...
        if op == "conv":
            ...
            exp.reshape(layer["h"], layer["w"], layer["c"])
