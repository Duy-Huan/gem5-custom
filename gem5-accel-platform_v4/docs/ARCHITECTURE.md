...
## Import model ONNX thật (v5)

Mục 6 và 7 (v3, v4) dùng JSON viết tay, tái tạo đúng cấu trúc framework
(block `c2f`, `bottleneck_mbv2`...). Để dùng được với **bất kỳ model CNN
thật nào** (không chỉ 4 model mẫu), `workloads/tools/onnx_to_json.py`
đọc trực tiếp đồ thị ONNX và tự sinh JSON tương thích.

### Vì sao không tái dùng cách "tuần tự hoá (h,w,c)" của Expander

JSON viết tay (mục 6/7) dựa vào việc `Expander` tự tính `(h,w,c)` tuần tự
qua từng block — cách này đúng vì ta *biết trước* cấu trúc block (một
nhánh chính, không rẽ). Một model ONNX thật bất kỳ có thể có nhánh
rẽ/skip connection/concat — một biến `(h,w,c)` tuần tự đơn không đủ để
theo dõi đúng. `onnx_to_json.py` giải quyết bằng cách **không tự đoán
shape** mà luôn tra cứu **shape inference thật của ONNX**
(`onnx.shape_inference.infer_shapes`) sau mỗi node — nghĩa là hiểu đúng
shape thật của đồ thị bất kể độ phức tạp rẽ nhánh, chỉ tự tính công thức
riêng cho phần thật sự cần (kích thước output của Conv, từ đúng
`pads`/`strides`/`dilations` — chính xác hơn giả định "pad=k//2" của
JSON viết tay) — và **luôn đối chiếu chéo** kết quả tự tính với shape suy
luận thật của ONNX, cảnh báo nếu lệch.

### Vấn đề "no-MAC op vẫn đổi shape" — và cách giải quyết bằng op `reshape`

Các phép không có MAC (pooling, `GlobalAveragePool`, `Flatten`) bị bỏ qua
(đúng, không gửi xuống NPU) — nhưng chúng **vẫn làm thay đổi kích thước
không gian/kênh**. Nếu bỏ qua hoàn toàn, `Expander` phía nhận JSON
(`model_to_npu_calls.py`) sẽ không biết shape đã đổi, tính sai `M` cho
lớp `linear`/`conv` tiếp theo (lỗi này **đã bắt được thật** khi viết test
— xem `workloads/tools/test_onnx_to_json.py` Test 1: `M` tính sai 64 thay
vì 1 sau `GlobalAveragePool`). Giải pháp: thêm một loại layer JSON mới,
`{"op": "reshape", "h":.., "w":.., "c":..}` — không sinh lệnh NPU nào,
chỉ đồng bộ lại trạng thái `(h,w,c)` phía `Expander`, y hệt cách RTL
integration ở mục trước dùng "hook nhỏ" thay vì viết lại toàn bộ máy
trạng thái.

### Grouped convolution tổng quát (không chỉ depthwise nhị phân)

`Expander.conv()` (dùng chung cho cả JSON viết tay lẫn ONNX) được mở rộng
nhận tham số `groups` tổng quát: `groups==Cin` (và `Cout==Cin`) → depthwise
thật (N=1/kênh, như trước); `1<groups<Cin` (kiểu ResNeXt) → tách thành
`groups` lệnh GEMM dày đặc độc lập, mỗi lệnh `Cin/groups → Cout/groups`
kênh — đúng bản chất toán học của grouped conv, không phải xấp xỉ.

### Đã kiểm chứng bằng 4 test tự động (không phải chạy thử qua loa)

`workloads/tools/test_onnx_to_json.py` tự dựng đồ thị ONNX **thật** (qua
`onnx.helper`, không mock) và assert khớp chính xác với đáp số tính tay:

1. **CNN tuần tự** (dense + depthwise + `GlobalAveragePool` + linear) —
   khớp chính xác 68.256 MAC, bắt được đúng bug "quên đồng bộ shape sau
   pooling" mô tả ở trên trong quá trình phát triển.
2. **Grouped conv** (`groups=4`, không depthwise) — khớp chính xác 73.728
   MAC.
3. **Residual connection** (`Add` nối 2 nhánh) — khớp chính xác 516.096
   MAC, xác nhận state đồng bộ đúng qua điểm rẽ nhánh.
4. **Round-trip MobileNetV2**: dựng lại đúng kiến trúc MobileNetV2 (Table
   2 của paper) dưới dạng đồ thị ONNX thật, convert qua
   `onnx_to_json.py`, và so với số liệu JSON viết tay đã đối chiếu paper ở
   mục 6 — kết quả khớp **tuyệt đối byte-for-byte**: 300.774.272 MAC, 53
   layer nguyên thuỷ, 7277 lệnh NPU, giống hệt không sai một đơn vị.

Đây là bằng chứng độc lập mạnh nhất trong toàn bộ nền tảng: hai con đường
hoàn toàn khác nhau (viết tay theo paper vs. tự động từ đồ thị ONNX) cho
ra đúng cùng một con số.

### Giới hạn đã biết

1. **Chỉ hỗ trợ kernel/stride vuông** (`kh==kw`, `sh==sw`) — model có
   kernel không vuông sẽ báo lỗi rõ ràng thay vì âm thầm tính sai.
2. **Giả định node đã được sắp topological** (đúng theo chuẩn ONNX, luôn
   đúng với model export từ framework chuẩn) — không tự tính lại thứ tự.
3. **Nhánh rẽ phức tạp** (nhiều nhánh có độ phân giải khác nhau hợp nhất
   qua `Concat`) dựa vào shape inference của ONNX cho đúng shape sau hợp
   nhất, nhưng chưa tự kiểm chứng với model có `Concat` đa nhánh thật
   (mới test `Add` 2 nhánh cùng shape) — nên dùng `--verbose` để tự kiểm
   tra log shape khi áp dụng cho model có kiến trúc dạng FPN/PAN phức tạp.
4. **Không tự nhận diện block cấp cao** (không tái tạo `c2f`/`bottleneck`)
   — xuất ra danh sách layer nguyên thuỷ phẳng, mất đi tên gọi block gốc
   nhưng không mất độ chính xác MAC.
